...
import InvalidMetatagsError from "./InvalidMetatagsError.js";
 ...
 */
export function isActivityDeclaration(node) {
    if (!Node.isClassDeclaration(node)) {
        return false;
    }

    const extendsExpression = node.getExtends();
    const extendsExpressionText = extendsExpression && extendsExpression.getText();
    const implementsExpression = node.getImplements();

    const hasCorrectExtends =
        implementsExpression.some(expression => expression.getText().includes("IActivityHandler")) ||
...
function getTagsFromJsDocs(jsDocs) {
    ...
    const tags = jsDocs
        .reduce((/** @type {JSDocTag[]} */ acc, jsDoc) => acc.concat(jsDoc.getTags()), [])
        .reduce(
            (acc, tag) => ({
                ...acc,
...
function validateMetatags(meta, itemType) {
    ...
    for (const pair of incompatibleMetatags) {
        if (
            Object.prototype.hasOwnProperty.call(meta, pair[0]) &&
            Object.prototype.hasOwnProperty.call(meta, pair[1])
        ) {
...
function getParameterMetadata(param, properties, inputType) {
    ...
    for (const paramProp of properties) {
        ...
        const paramMeta = {
            clientOnly: Object.prototype.hasOwnProperty.call(paramTags, "clientOnly") || undefined,
            ...
            isHidden: Object.prototype.hasOwnProperty.call(paramTags, "hidden") || undefined,
            isRequired: Object.prototype.hasOwnProperty.call(paramTags, "required") || undefined,
            name: paramName,
            noExpressions: Object.prototype.hasOwnProperty.call(paramTags, "noExpressions") || undefined,
            onlineOnly: Object.prototype.hasOwnProperty.call(paramTags, "onlineOnly") || undefined,
            placeholder: paramTags.placeholder,
            serverOnly: Object.prototype.hasOwnProperty.call(paramTags, "serverOnly") || undefined,
            supportedApps: parseCommaSeparatedValues(paramTags.supportedApps),
            typeName: paramType,
            unsupportedApps: parseCommaSeparatedValues(paramTags.unsupportedApps),
        };
...
function getActivityMetadata(declaration, suiteUuid) {
    ...
    const inputs = inputType ? getParameterMetadata(executeMember, inputType.getProperties(), "activity input") : {};
    const outputs = returnType
        ? getParameterMetadata(executeMember, returnType.getProperties(), "activity output")
        : {};
    ...
    return {
        ...
        clientOnly: Object.prototype.hasOwnProperty.call(activityTags, "clientOnly") || undefined,
        description: activityTags.description,
        defaultName: activityTags.defaultName,
        deprecated: activityTags.deprecated,
        displayName: activityTags.displayName || toPascalCase(activityName),
        helpUrl: activityTags.helpUrl,
        inputs,
        isHidden: Object.prototype.hasOwnProperty.call(activityTags, "hidden") || undefined,
        onlineOnly: Object.prototype.hasOwnProperty.call(activityTags, "onlineOnly") || undefined,
        outputs,
        suite,
        serverOnly: Object.prototype.hasOwnProperty.call(activityTags, "serverOnly") || undefined,
        supportedApps: parseCommaSeparatedValues(activityTags.supportedApps),
        unsupportedApps: parseCommaSeparatedValues(activityTags.unsupportedApps),
    };
}
...
function getElementMetadata(registrationDeclaration, suiteUuid) {
    ...
    // @ts-ignore
    const registrationObjectExpression = registrationDeclaration.getInitializer();

    /**
     * @param {ObjectLiteralExpression} regObject
     */
    function getElementId(regObject) {
        const idPropertyAssignment = regObject.getPropertyOrThrow("id").getSymbolOrThrow().getValueDeclarationOrThrow();

        // Note that shorthand property assignment won't pass this check.
        if (!Node.isPropertyAssignment(idPropertyAssignment)) {
            throw new Error(`Must directly assign value to "id" property of element registration. Ex. id: "foo".`);
        }

        const idStringLiteral = idPropertyAssignment.getInitializerOrThrow();
        if (!Node.isStringLiteral(idStringLiteral)) {
            throw new Error(`Value of "id" property of element registration must be a string literal. Ex. id: "foo".`);
        }
    ...
    function getComponentDeclarationFromPropertyValue(propValue) {
        if (Node.isVariableDeclaration(propValue)) {
            ...
            if (!callSignature) {
                throw new Error(`"component" property of element registration isn't a function or class component.`);
            }
    ...
    function getInputsMetadata(componentDeclaration) {
        ...
        if (Node.isClassDeclaration(componentDeclaration)) {
            ...
            const componentTypeArgs = componentDeclaration.getExtendsOrThrow().getTypeArguments();
            ...
            const elementPropsMembers = componentPropsType
                ...
                .getMembers();
            return getParameterMetadata(componentPropsType, sanitizeMembers(elementPropsMembers), "activity input");
        } else {
            ...
            return getParameterMetadata(componentPropsParam, sanitizeMembers(elementPropsMembers), "activity input");
        }
    }

    const componentPropertyAssignment = registrationObjectExpression?.getPropertyOrThrow("component");
    if (!Node.isPropertyAssignment(componentPropertyAssignment)) {
        throw new Error('"component" property of element registration is not a property assignment.');
    }
    ...
    const componentDeclaration = getComponentDeclarationFromPropertyValue(componentPropertyValue);
 ...
 * @param {string} suiteUuid
 */
export function getProjectMetadata(projectExportsFile, suiteUuid) {
    const activities = [];
    const elements = [];

    for (const [, declarations] of projectExportsFile.getExportedDeclarations()) {
