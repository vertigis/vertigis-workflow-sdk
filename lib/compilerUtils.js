...
function getParameterMetadata(param, properties) {
    ...
    for (const paramProp of properties) {
        ...
        const paramMeta = {
            ...
            deprecated:
                Object.prototype.hasOwnProperty.call(paramTags, "deprecated") ||
                undefined,
...
function getElementMetadata(registrationDeclaration, suiteUuid) {
    ...
    function getComponentDeclarationFromPropertyValue(propAssignment) {
        if (Node.isVariableDeclaration(propAssignment)) {
            const componentFunctionDeclaration = propAssignment
                .getType()
                .getCallSignatures()[0]
                .getDeclaration();
            ...
            if (Node.isCallSignatureDeclaration(componentFunctionDeclaration)) {
                throw new Error(
                    `Component in element registration was declared using a call signature declaration which is not supported. 
